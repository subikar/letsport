<?php  
	...
	class Project extends Master 
	{
		...
		{
			global $db;
            $sql="SELECT p.project_name,p.deadline,p.status as project_status,t.* ,u.name as modifier_name FROM #__task as t LEFT JOIN #__project as p ON t.project_id=p.id LEFT JOIN #__users as u ON t.modified_by=u.uid WHERE t.project_id =".$db->quote($project_id)." GROUP BY t.id ORDER BY modified_date DESC";
            $db->setQuery($sql);
		...
		function editDeadline()
		{
			global $db,$template;
			$project_id = IRequest::getInt("project_id");
			$sql = "SELECT * FROM #__project WHERE  id=".$db->quote($project_id);
			$db->setQuery($sql);
			$projectDetails = $db->loadObjectList();
			$template->assignRef('projectDetails',$projectDetails[0]);			
		}
		function savedeadline()
		{
			global  $db,$template, $Config,$mainframe,$my;
			$post = IRequest::get("post");
			$update1 = "UPDATE #__project SET deadline=".$db->quote($post['deadline'])." WHERE id=".$db->quote($post['project']);
			$db->setQuery($update1);
			echo "<script> parent.jQuery.fn.colorbox.close();</script>";
			echo "<script>window.parent.location.href='".$Config->site.'myprojects#project/'.$post['project']."'</script>";
		...
		function closeproject()
		{
			...
			$post = IRequest::get('post');
			if(isset($post['intime']) && $post['intime'] == 1)
			{
				$star = 5;
				$mail_point = 0;
			}
			...
			$sql = "SELECT p.project_name,t.id,t.task_name,t.point,tm.creator_id,u.name,u.email FROM (#__task as t LEFT JOIN #__project as p ON t.project_id=p.id) LEFT JOIN (#__task_management as tm LEFT JOIN #__users as u ON tm.creator_id=u.uid) ON t.id=tm.task_id WHERE t.project_id=".$db->quote($post['project_id'])." AND tm.task_hour > 0 GROUP BY tm.creator_id";
			$db->setQuery($sql);
			...
			{
				$taskIds[] = $value->id;
				$this->post =array("task_id"=>$value->id, "employee_id"=>$value->creator_id, "star"=>$star,"create_date"=>date('Y-m-d H:i'));
				...
				$mailArray = array("details"=>$value,"Star"=>$star);
				$this->closeProjectMail($mailArray);
			}
			$taskIds = array_unique($taskIds);
			$taskIds = implode(",",$taskIds);
			$update1 = "UPDATE #__task SET status=1".$point." WHERE id IN(".$taskIds.")";
			$db->setQuery($update1);
			$update = "UPDATE #__project SET status=1 WHERE id=".$db->quote($post['project_id']);
			$db->setQuery($update);
			echo "<script>parent.Project.displayPage();</script>";
			echo "<script> parent.jQuery.fn.colorbox.close(); </script>";
		}
		function closeProjectMail($mailDetails)
		{
			global $Config;	
			$mailer=new IMail;
			ob_start();
			include(IPATH_ROOT."/mail_inc/closeProject.inc");
			$message = ob_get_clean();
			$message=str_replace('{task_name}',$mailDetails["details"]->task_name,$message);
			$message=str_replace('{user_name}',$mailDetails["details"]->name,$message);
			$message=str_replace('{project_name}',$mailDetails["details"]->project_name,$message);
			$message=str_replace('{point}',$mailDetails["details"]->point,$message);
			$message=str_replace('{star}',$mailDetails["Star"],$message);
			$mailer->From="info@itcslive.com";
			$mailer->Subject="Project Sucessfully Completed";
			$mailer->To = $mailDetails['details']->email;
			$mailer->Message = $message; 
			$mailer->send();
		}
		...
		function saveassignProject()
		{
			...
			echo "<script> parent.jQuery.fn.colorbox.close();</script>";
			echo "<script>window.parent.location.href='".$Config->site.'myprojects#project/'.$project_id."'</script>";
		}
		...
		function addtask()
		{
			...
			if($task_id != 0)
			{
				$sql = "SELECT pr.id,pr.project_name,t.point FROM #__project as pr LEFT JOIN #__task as t ON pr.id=t.project_id WHERE t.id=".$db->quote($task_id)." and t.project_id > 0";
				$db->setQuery($sql);
				$projectDetails = $db->loadObjectList();
				$Refproject = array("projectDetails"=>$projectDetails,"task_id"=>$task_id);
				$template->assignRef('Refproject',$Refproject);
			}
		}
		...
		function savetask()
		{
			...
			$this->post =array("project_id"=>$project_id, "task_name"=>$post["task_name"], "creator_id"=>$my->uid,"create_date"=>date('Y-m-d H:i'),"modified_date"=>date('Y-m-d H:i'),"modified_by"=>$my->uid,"point"=>$post["point"]);
			parent::bind('task');
			parent::save();
			$taskId =  $db->insertid();
			
			if(isset($post['total_point']) && $post['total_point'] > 0) :
				$remaining_point = $post['total_point'] - $post['point'];
				$update = "UPDATE #__task SET point=".$db->quote($remaining_point)." WHERE id=".$db->quote($task_id);
				$db->setQuery($update);
			endif;
		...
		function taskContent()
		{
			...
			$employeeWorking = $db->getOne();
			 ...
			 if(strtolower($my->usertype)=="customer"): 
			$sql = "SELECT tm.*,u.name FROM #__task_management as tm LEFT JOIN #__users as u on tm.creator_id=u.uid WHERE tm.task_id=".$db->quote($task_id)." AND (tm.internal_comment = 0 OR tm.internal_comment = 2) ORDER BY tm.id DESC";
			elseif(strtolower($my->usertype)=="employee"):
			$sql = "SELECT tm.*,u.name FROM #__task_management as tm LEFT JOIN #__users as u on tm.creator_id=u.uid WHERE tm.task_id=".$db->quote($task_id)." AND tm.internal_comment != 2 ORDER BY tm.id DESC";
			else:
			...
			//code for format hrs and minutes
			foreach($taskContent as $eachtask):
				if((float)$eachtask->task_hour > 0){
					$eachtask->showhrs=1;
					$totalMinutes=($eachtask->task_hour * 60);
					$format = '%02d hours %02d minutes';
					settype($totalMinutes, 'integer');
					$hours = floor($totalMinutes / 60);
					$minutes = ($totalMinutes % 60);
					$eachtask->showhrsformat = sprintf($format, $hours, $minutes);
				}
			endforeach; 
		...
		{		
			...
			if($post['attachment'] != '')
			{
				...
				foreach($Attachment as $value):
					...
					if(strtolower($extension) == 'txt' || strtolower($extension) == 'ai')
		...
		function saveComment()
		{
			...
			if(strtolower($my->usertype) != "employee")
			{
				...
				unset($post['task_minute']);
			}
			else
			{
				if($post['task_hour'] > 0 || $post['task_minute'] > 0){
					$minHrs=((int)$post['task_minute'] > 0) ? ((int)$post['task_minute'] / 60) : 0 ;
					$post['task_hour']=(int)$post['task_hour'] + $minHrs;
				} else {
					$post['task_hour']=0;
				}
			}
			...
			{
				$taskDetails->total_hour = $taskDetails->total_hour + (float)$post['task_hour'];
			...
			elseif((int)$post['internal_comment'] == 2):
			$where=" WHERE pr.project_id=".$db->quote($taskDetails->project_id)." AND u.usertype !='employee'";
			else:
			$where=" WHERE pr.project_id=".$db->quote($taskDetails->project_id);
			endif;
			
			$sql2 = "SELECT u.email,u.name,u.password FROM #__users as u LEFT JOIN #__project_relation pr ON u.uid=pr.relation_id".$where;
			$db->setQuery($sql2);
			...
			//code for format hrs and minutes
				if((float)$post['task_hour'] > 0){
					$showhrs=1;
					$totalMinutes=($post['task_hour'] * 60);
					$format = '%02d hours %02d minutes';
					settype($totalMinutes, 'integer');
					$hours = floor($totalMinutes / 60);
					$minutes = ($totalMinutes % 60);
					$showhrsformat = sprintf($format, $hours, $minutes);
				}
			
			$taskHourHtml= ($showhrs == 1) ? "<span style=\"float:right; color:#acadad; font-size:11px\">Task Hour:".$showhrsformat."</span>" : "";
			$removeLink= (strtolower($my->usertype)=="admin") ? "<div style=\"float:right; color:#000;\"><a href=\"javascript:void(0);\" onclick=\"Project.RemoveTaskComment(".(int)$taskContentId.");\"><i class=\"fa fa-trash-o_1 fa-right\"></i></a></div>" : "" ;
			$commentStar=(((int)$post['internal_comment'] == 1) ? '<span class="internal_team" title="Internal Comment"><i class="fa fa-star"></i></span>': (((int)$post['internal_comment'] == 1) ? '<span class="admin_team" title="Administrative Comment"><i class="fa fa-star"></i></span>' : ''));
			
			$html="<div id=\"comment_box_".$taskContentId."\"><div class=\"defaultBoxLine5\" style=\"width:96%;\"><div class=\"tb-comment-list\"><div class = \"tb-comment-header\"><div class=\"user_avatar\"><span class=\"tb-avatars-initials undefined\"><i class=\"icon icon-avater\"></i></span></div><div class=\"tb-comment-info tb-comment-info--online\"><a class=\"js-mention-link tb-comment-author\">". $my->name."</a></div>".$commentStar.$removeLink."</div><div><span class=\"tb-comment-meta-info\">". date("j F Y h:i A")."</span>".$taskHourHtml."</div><div class=\"tb-comment-text\"><p>".nl2br($post["task_content"])."</p></div></div></div><div class=\"line\"></div></div>";
		...
		{	
			...
			foreach($mailDetails['users'] as $value)
			{
				...
				{
					ob_start();
					include(IPATH_ROOT."/mail_inc/ReplyMail_ForProject.inc");
					$message = ob_get_clean();
					...
					$message=str_replace('{user_name}',$value->email,$message);
					$message=str_replace('{user_password}',$value->password,$message);	
		...
		function getDefaultTaskDetails()
		{
			...
			$sql = "SELECT u.uid,u.name,t.id,t.task_name,t.point,sum(tm.task_hour) as task_hour,tm.creator_id,tm.create_date FROM #__task_management as tm LEFT JOIN #__task as t ON tm.task_id=t.id LEFT JOIN #__users as u ON tm.creator_id=u.uid ".$where." AND EXTRACT(month FROM tm.create_date)=".$db->quote($month)." GROUP BY tm.task_id,tm.creator_id,DATE(tm.create_date)"; 
			$db->setQuery($sql);
			$taskDetails = $db->loadObjectList();
			...
			for($j=0;$j < count($values); $j++):
				...
				$ret_array[$k]->task_hour = $taskHour;
				$ret_array[$k]->uid = $values[$j]->uid;
				$ret_array[$k]->id = $values[$j]->id;
				$ret_array[$k]->name = $values[$j]->name;
				$ret_array[$k]->task_name = $values[$j]->task_name;
				$k++;
			endfor;
			$ret_array = array_unique($ret_array, SORT_REGULAR);
			$totalTaskHour = 0;
			foreach($ret_array as $value):
				$totalDetails[]="<tr><td>".$value->task_name."</td><td><a href=javascript:void(0) onclick='Calendar.userevent(".$value->uid."); id'>".$value->name."</a></td><td>".$value->task_hour."hr</td></tr>";
				$totalTaskHour = $totalTaskHour + $value->task_hour;
			endforeach;
			$taskHtml['total_hour'] = $totalTaskHour;
		...
		function getEmployeeName()
		{
			...
			$sql = "SELECT uid as value,name as text FROM #__users WHERE uid=".$db->quote($user_id);
			$db->setQuery($sql);
			$RefUsers = $db->LoadObjectList();
			$RefUsers = $RefUsers[0];
			print_r(json_encode($RefUsers)); exit;
		...
		function getUser_fromAjax()
		{
			...
			if(trim($userHint)!="")
			{
				...
				$sql="SELECT uid as value,name as text FROM #__users WHERE".$where." AND LOWER(name) LIKE '%".$userHint."%' AND (usertype='employee' OR usertype='telecaller' OR usertype='fieldexecutive')";				
				$db->setQuery($sql);
				$RefUsers = $db->LoadObjectList();
			}
			print_r(json_encode($RefUsers)); exit;
