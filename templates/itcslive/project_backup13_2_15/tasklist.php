<?php $accessForUser=array("customer","telecaller","Admin","employee"); ?>
      ...
      <div class="boiteHeader">
		 ...
		 <ol>		
		  <?php if(in_array($my->usertype,$accessForUser)): ?>
            <li> <a title="Add Task" onclick='jQuery.colorbox({href: "<?php echo $Config->site."addtask?project_id=".$project_id; ?>", iframe:true, width:"520px", height:"500px", scrolling:false, open:true, overlayClose:true, title:"Add Task"});' href="javascript:void(0);" class="icon"> <span class="fa add"></span> Add Task </a></li>
			<?php endif; ?>
         </ol>
		 		 	
      </div>
	 <?php if((strtolower($my->usertype) == 'admin' || strtolower($my->usertype) == 'telecaller' ) && (int)$project_id > 0): ?> 
     
	 <div class="assign"> <i class="fa fa-arrows_1"></i> <a href="<?php echo $Config->site.'assignproject?project_id='.$project_id; ?>" class="assignproject assign_1">Assign Project</a>&nbsp;</div><?php endif; ?>
	  ...
	  <h3 class="task_ongoing"><b>Completed Tasks</b><a style="text-align:right;" href="javascript:void(0);" onclick="jQuery('#all_completed_task').toggle('slow');"><i class="fa fa-arrow-circle-o-down"></i></a></h3>
         <div class="defaultBoxLine">
			...
			<?php if(count($CompletedTaskList) > 0): ?>
			   ...
			   <span><small>Last Modified By: <?php echo $task->modifier_name; ?></small></span><br />
			   <span><small>Last Modified : <?php echo $task->updated_before; ?> ago </small></span><br />
			    <!--<span><small>Time Taken: <?php// echo $task->total_hour; ?> </small></span>-->
			   </li>
               <input type="hidden" name="task_id" id="task_id" value="<?php  echo $task->id;  ?>" />
               <?php endforeach; ?>
			   <?php else: ?>
