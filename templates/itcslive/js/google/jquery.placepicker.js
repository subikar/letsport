(function ( jQuery, window) {
  ...
  function PlacePicker(element, options) {
    ...
    function setMapElement() {
      ...
      mapElement = jQuery(options.map).get(0);

      if (!mapElement) {
        if (options.mapContainerId) {
          mapElement = jQuery("#" + options.mapContainerId + " .placepicker-map").get(0);
        }
      }
    ...
    function initMap() {
      ...
      // refreshed.
      jQuery(mapElement).parent().on("show.bs.collapse", function(e) {
        jQuery(e.target)
          .css("display", "block")
          // Fix map icons
    ...
    function init() {
      ...
      if (!element.value) {
        var lat = options.latitude || jQuery(options.latitudeInput).prop("value");
        var lng = options.longitude || jQuery(options.longitudeInput).prop("value");
        if (lat && lng) {
    ...
    function setPlace(place, updateMap) {
      ...
      jQuery(options.latitudeInput).prop("value", pos.lat());
      jQuery(options.longitudeInput).prop("value", pos.lng());
  ...
  jQuery.fn[pluginName] = function(options) {
    ...
    return this.each(function() {
      if (!jQuery(this).data(pluginName)) {
          jQuery(this).data(pluginName, new pluginClass(this, jQuery.extend({}, defaults, options, jQuery(this).data())));
      }
      return jQuery(this);
    });
  };
