...
error_reporting(0);
	...
	$accessForUser=array("employee","Admin");
?>
<div class="padDiv row5 box_1">
	<div class="boiteHeader">
		...
		<ol>
			...
			<li>
				<a href="<?php echo $Config->site."addtask?task_id=".$mainTask->id; ?>" class="addtaskemployee">
					<span class="fa add"></span>Add Task
				</a>
			</li>
			<?php endif; ?>
		</ol>
    ...
    <div class="line"></div>
		...
		<div id="total_point">Total Point : <?php echo $mainTask->point; ?></div>
			...
			<form name="taskComment" id="taskComment" method="post" onsubmit="Project.taskComment(this);">
				 ...
				 <ul id="allAttechFile" class="attach_file"></ul>
				  <?php if(strtolower($my->usertype)=="customer" || strtolower($my->usertype) == 'telecaller' || strtolower($my->usertype) == 'admin' ): ?>
				  <span><input type="checkbox" name="internal_comment" id="internal_comment2" value="2">Customer Comment</span> 
				  <?php endif; ?>
				  <?php if(strtolower($my->usertype)!="customer"): ?>
				<span><input type="checkbox" name="internal_comment" id="internal_comment1" value="1">Comment For Internal Team.</span> 
				<?php endif; ?>
			   <?php if(strtolower($my->usertype)=="employee"): ?>
			   <span class="task tasktime">
			   <input type="text" name="task_hour" id="task_hour" placeholder="Task Hrs"/>:
			   <input type="text" name="task_minute" id="task_minute" placeholder="Task Min"/>
			   </span> 
				 <?php endif; ?>				 
			...
			<ul id="contentbox" class="comment_list">
				...
				<div id="comment_box_<?php echo $Task->id; ?>">
					...
					<?php  echo ((int)$Task->internal_comment==1) ? '<span class="internal_team" title="Internal Comment"><i class="fa fa-star"></i></span>': (((int)$Task->internal_comment==2) ? '<span class="admin_team" title="Administrative Comment"><i class="fa fa-star"></i></span>' : ''); ?>
					...
					<?php if($Task->showhrs == 1): ?>
					Task Hour: <?php echo $Task->showhrsformat;  endif; ?>
					</span>
					</div>
...
<script>
	jQuery(document).ready(function()
	{
		...
		jQuery(".addtaskemployee").colorbox({iframe:true, width:"60%", height:"75%",title:"Add Task",scrolling:false}); 
	});
</script>